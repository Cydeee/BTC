/* ─────────────────────────────────────────────────────────
   ...
   • Works in Netlify Edge (Deno)       → no proxy branch runs
   • Works in Node with HTTPS_PROXY     → dynamic proxy agent
   ───────────────────────────────────────────────────────── */
...
async function safeJson(url) {
  // In Netlify Edge (Deno) there's no need—and no support—for Node proxy code.
  if (process.env.HTTPS_PROXY && typeof Deno === 'undefined') {
    const { default: HttpsProxyAgent } = await import('https-proxy-agent');
    return fetch(url, { agent: new HttpsProxyAgent(process.env.HTTPS_PROXY) })
             .then(r => r.ok ? r.json() : Promise.reject(new Error(`HTTP ${r.status}`)));
  }
  // Default fetch (Edge runtime or proxy not set)
...
export default async function handler(request) {
  ...
  const wantsJson = new URL(request.url).pathname.endsWith('/data.json');
  ...
  try {
    ...
    const body = wantsJson
      ...
      : `<!doctype html><html lang="en"><body><pre id="dashboard-data">${JSON.stringify(payload, null, 2)}</pre></body></html>`;
...
async function buildDashboardData() {
  ...
  /* H: Sentiment */
  try {
    const fng = await safeJson('https://api.alternative.me/fng/?limit=1');
    const d   = fng.data?.[0]; if (!d) throw new Error('FNG missing');
