...
// Base URL for your Netlify Function proxy
const PROXY_BASE = "https://btcsignal.netlify.app/.netlify/functions/proxy?path=";

// Fetch JSON via your proxy
...
const rsi = (a, p) => {
  ...
  for (let i = p + 1; i < a.length; i++) {
    const d0 = a[i] - a[i - 1];
    au = (au * (p - 1) + Math.max(d0, 0)) / p;
    ad = (ad * (p - 1) + Math.max(-d0, 0)) / p;
  }
...
async function buildDashboardData() {
  ...
  const out = {
    dataA: {}, dataB: null, dataC: {},
    dataD: {}, dataE: null, dataF: null,
    dataG: null, dataH: null, errors: []
  };
  ...
  for (const tf of ["15m","1h","4h","1d"]) {
    try {
      ...
      const h = kl.map(r => +r[2]);
      const l = kl.map(r => +r[3]);
      const last = c.at(-1) || 1;
  ...
  try {
    ...
    const oiΔ = ((+oiNow.openInterest - +oiH[0].sumOpenInterest) / +oiH[0].sumOpenInterest * 100).toFixed(1);
  ...
  try {
    ...
    for (const [lbl,hrs] of Object.entries(wins)) {
      ...
      const ag = await proxyJson(`/fapi/v1/aggTrades?symbol=${S}&startTime=${start}&endTime=${end}&limit=1000`);
      let cvd=0; ag.forEach(t=>{ cvd += t.m ? -t.q : +t.q; });
  ...
  // G: Macro
  try {
    const gobj = await proxyJson(`/api/v3/global`);
    const g = gobj.data;
    out.dataG = {
  ...
  // H: Sentiment
  try {
    const fobj = await proxyJson(`/api/fng/?limit=1`.replace("/api/","/api.alternative.me/"));
    const f0 = fobj.data?.[0];
