/* netlify/edge-functions/data.js
   Always‑proxy version (Edge‑safe)                             */

const SYMBOL        = 'BTCUSDT';
const LIMIT         = 250;
const PROXY_BASEURL = 'https://fr-proxy.yourdomain.com/proxy/';  // ← change to your domain

/* ──────────────────────────────────────────
   Generic fetch helper (uses proxy URL)
────────────────────────────────────────── */
async function safeJson (url) {
  const r = await fetch(PROXY_BASEURL + encodeURIComponent(url));
...
export default async function handler(request) {
  ...
  try {
    ...
    const headers = wantJson ? jsonHeaders() : htmlHeaders();
...
async function buildDashboardData() {
  ...
  /* H – Sentiment */
  try {
    const fg=await safeJson('https://api.alternative.me/fng/?limit=1'), d=fg.data?.[0]; if(!d) throw new Error('FNG missing');
