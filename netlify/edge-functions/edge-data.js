/* netlify/edge-functions/data.js
   Dashboard blocks:
   A indicators | B derivatives+liq | C ROC | D volume+CVD
...
async function buildDashboardData () {
  ...
  try{
    ...
    for(const [lbl,hrs] of Object.entries(win)){
      const end=Date.now(), start=end-hrs*3600_000;
  ...
  try{
    ...
    const vFlag=out.dataD.relative["15m"], vScore=vFlag==="very high"?2:vFlag==="high"?1:0;
  ...
  /* F – VPVR + live price + intraday levels */
  try{
    const tfBars=i=>safeJson(`https://api.binance.com/api/v3/klines?symbol=${SYMBOL}&interval=${i}`);
    const vp=b=>{
      const bucket={}; b.forEach(x=>{
        const px=(+x[2]+ +x[3]+ +x[4])/3, key=Math.round(px/100)*100;
        bucket[key]=(bucket[key]||0)+ +x[5];
      });
      const poc=+Object.entries(bucket).sort((a,b)=>b[1]-a[1])[0][0];
      return { poc, buckets:bucket };
    };
    out.dataF={ vpvr:{ "4h":vp(await tfBars("4h&limit=96")),
                       "1d":vp(await tfBars("1d&limit=30")),
                       "1w":vp(await tfBars("1w&limit=12")) } };

    /* live price */
    const last1m = await safeJson(`https://api.binance.com/api/v3/klines?symbol=${SYMBOL}&interval=1m&limit=1`);
    const price  = +last1m[0][4];
    out.dataF.price=+price.toFixed(2);

    /* levels */
    const dBars   = await safeJson(`https://api.binance.com/api/v3/klines?symbol=${SYMBOL}&interval=1d&limit=2`);
    const yHigh=+dBars[0][2], yLow=+dBars[0][3], yClose=+dBars[0][4];
    const pivot=(yHigh+yLow+yClose)/3, R1=2*pivot-yLow, S1=2*pivot-yHigh;

    const hhllBars = await safeJson(`https://api.binance.com/api/v3/klines?symbol=${SYMBOL}&interval=1h&limit=20`);
    const HH20=Math.max(...hhllBars.map(b=>+b[2]));
    const LL20=Math.min(...hhllBars.map(b=>+b[3]));
    ...
    let vs=0,pv=0,pv2=0;
    vBars.forEach(b=>{ const v=+b[5], px=(+b[1]+ +b[2]+ +b[3]+ +b[4])/4;
                       vs+=v; pv+=px*v; pv2+=px*px*v; });
    const vwap=pv/vs, sigma=Math.sqrt(Math.max(pv2/vs - vwap*vwap,0));
  ...
  try{
    ...
    const d=fg.data?.[0]; if(!d) throw new Error("FNG missing");
...
export default async function handler(req){
  ...
  try{
    ...
    if(wantJson){
      return new Response(JSON.stringify(payload),{ headers: hdrJSON() });
    }else{
      return new Response(`<!DOCTYPE html><html><body><pre id="dashboard-data">${
        JSON.stringify(payload, null, 2)
      }</pre></body></html>`,{ headers: hdrHTML() });
    }

  }catch(e){
    console.error("Edge error:",e);
    return new Response("Service unavailable", { status:500, headers:hdrHTML() });
  }
}
