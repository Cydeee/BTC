/*  Netlify Node 18 Edge Function – data.js
    ...
    Added: 4 h ADX-14, abs./percentile OI, sessionRelVol, cycle-anchored VWAP, swing detection.
*/
...
function adx(h,l,c,p=14){
  ...
  const tr14=smooth(tr), plus14=smooth(dmP), minus14=smooth(dmM), dx=[];
...
async function buildDashboardData() {
  ...
  try {
    ...
    const oiHist = await safeJson(`https://api.binance.com/api/v3/futures/data/openInterestHist?symbol=${SYMBOL}&period=1h&limit=500`);
  ...
  /* F – VPVR, Levels, Live Price (added avwapCycle) */
  try {
    ...
    let num=0, den=0;
    daily.forEach(r=>{ const p=(+r[2]+ +r[3]+ +r[4])/3; const v=+r[5]; num+=p*v; den+=v; });
    out.dataF.avwapCycle = +(num/den).toFixed(2);

    // ───────── Swing High/Low Detection ─────────
    { 
      const candles = await safeJson(`https://api.binance.com/api/v3/klines?symbol=${SYMBOL}&interval=1m&limit=120`);
      const swings = { H1:null, H2:null, L1:null, L2:null };
      const highs = [], lows = [];
      for (let i = 6; i < candles.length - 6; i++) {
        ...
        for (let j = i - 6; j <= i + 6; j++) {
          const h = +candles[j][2], l = +candles[j][3];
          if (h > high) isHigh = false;
          if (l < low) isLow = false;
          windowLow = Math.min(windowLow, l);
          windowHigh = Math.max(windowHigh, h);
        }
        if (isHigh && (high - windowLow)/windowLow >= 0.0015) highs.push({idx:i,price:high});
        if (isLow && (windowHigh - low)/windowHigh >= 0.0015) lows.push({idx:i,price:low});
      }
      if (highs.length >= 2 && (lows.length === 0 || highs[highs.length-1].idx > lows[lows.length-1].idx)) {
        const recent = highs.slice(-2);
        swings.H1 = recent[0].price;
        swings.H2 = recent[1].price;
      } else if (lows.length >= 2) {
        const recent = lows.slice(-2);
        swings.L1 = recent[0].price;
        swings.L2 = recent[1].price;
      }
      let neckline = null, neckBreak = false;
      const last = candles.length - 1;
      const prevClose = +candles[last-1][4], currClose = +candles[last][4];
      if (swings.H1 != null && swings.H2 != null) {
        const recentH = highs.slice(-2);
        const i1 = recentH[0].idx, i2 = recentH[1].idx;
        const segment = candles.slice(i1, i2+1);
        neckline = Math.min(...segment.map(c=>+c[3]));
        neckBreak = prevClose > neckline && currClose < neckline;
      } else if (swings.L1 != null && swings.L2 != null) {
        const recentL = lows.slice(-2);
        const i1 = recentL[0].idx, i2 = recentL[1].idx;
        const segment = candles.slice(i1, i2+1);
        neckline = Math.max(...segment.map(c=>+c[2]));
        neckBreak = prevClose < neckline && currClose > neckline;
      }
      out.dataF.swings = swings;
