/*  Netlify Node 18 Function  –  data.js
    ...
    HTTPS_PROXY respected via HttpsProxyAgent.
*/
...
async function buildDashboardData() {
  ...
  try{
    ...
    for(const [lbl,hours] of Object.entries(win)){
      ...
      out.dataD[lbl]={ bullVol:+bull.toFixed(2), bearVol:+bear.toFixed(2),
  ...
  try{
    ...
    const vp=bars=>{const bkt={};bars.forEach(b=>{const px=(+b[2]+ +b[3]+ +b[4])/3,key=Math.round(px/100)*100;bkt[key]=(bkt[key]||0)+ +b[5];});const poc=+Object.entries(bkt).sort((a,b)=>b[1]-a[1])[0][0];return{poc,buckets:bkt};};
    out.dataF={ vpvr:{ "4h":vp(h4),"1d":vp(d1),"1w":vp(w1)} };

    /* Structure extras */
    const y=d1.at(-2);
    if(y){
      const [yHigh,yLow,yClose]=[+y[2],+y[3],+y[4]];
      const pivot=(yHigh+yLow+yClose)/3, R1=2*pivot-yLow, S1=2*pivot-yHigh;

      /* HH20 / LL20 from last 20 one‑hour bars */
      ...
      const variance=pv2Sum/volSum - vwap*vwap;
      const sigma=Math.sqrt(Math.max(variance,0));
      const vwUp=vwap+sigma, vwDn=vwap-sigma;

      out.dataF.levels={
        pivot:+pivot.toFixed(2), R1:+R1.toFixed(2), S1:+S1.toFixed(2),
        HH20:+HH20.toFixed(2), LL20:+LL20.toFixed(2),
        vwap:+vwap.toFixed(2), vwapUpper:+vwUp.toFixed(2), vwapLower:+vwDn.toFixed(2)
      };
    }
...
export async function handler() {
  try{
    ...
    return { statusCode:200, headers:{ "Content-Type":"application/json; charset=utf-8",
      "Cache-Control":"public, max-age=0, must-revalidate" }, body:JSON.stringify(payload) };
  }catch(err){
