/*  Netlify Node 18 Function  –  data.js
    ...
    All outbound requests respect HTTPS_PROXY, e.g.
      http://botuser:toto92@164.132.56.117:3128
*/

import { HttpsProxyAgent } from "https-proxy-agent";
import fetch from "node-fetch";          // honours agent on Netlify
...
async function buildDashboardData() {
  ...
  const LIMIT  = 250;
  const out = {
    dataA:{}, dataB:null, dataC:{}, dataD:{}, dataE:null, dataF:null, dataG:null, dataH:null, errors:[]
  };
  ...
  try {
    ...
    if (yesterday) {
      ...
      const HH22=Math.max(...d1.map(b=>+b[2]));
      const LL22=Math.min(...d1.map(b=>+b[3]));

      const midnight=new Date(); midnight.setUTCHours(0,0,0,0);
      const vw1 = await safeJson(`https://api.binance.com/api/v3/klines?symbol=${SYMBOL}&interval=1m&startTime=${midnight.getTime()}&limit=1440`);
      let vSum=0,pvSum=0; vw1.forEach(b=>{ const vol=+b[5]; const px=(+b[1]+ +b[2]+ +b[3]+ +b[4])/4; vSum+=vol; pvSum+=px*vol; });
      const vwap = pvSum/vSum;

      out.dataF.levels = {
        pivot:+pivot.toFixed(2), R1:+R1.toFixed(2), S1:+S1.toFixed(2),
        HH22:+HH22.toFixed(2), LL22:+LL22.toFixed(2), vwap:+vwap.toFixed(2)
      };
    }
