/*  netlify/edge-functions/data.js               ──────────────────────────────
    Node 18 Edge Function – ES-module syntax
    Builds the BTC dashboard JSON with:
      · TA block (EMA, RSI, ATR%, MACD-hist, 4 h ADX-14)
      · Derivatives block (funding-Z, abs OI, 30-day OI-percentile, Δ-24 h OI)
      · Volume / CVD block + session-relative volume
      · Stress-index block
      · Structure block (VPVR, live price, cycle-anchored VWAP, pivots, VWAP-band)
      · Macro + sentiment
    All outbound requests honour HTTPS_PROXY if set.
*/

import fetch from "node-fetch";
import { HttpsProxyAgent } from "https-proxy-agent";

// Edge-functions are ESM, so import-based “node-fetch” v3 works.
const AGENT = process.env.HTTPS_PROXY
  ? new HttpsProxyAgent(process.env.HTTPS_PROXY)
  : undefined;

/* ---------- tiny wrapper so every fetch goes through the proxy ----------- */
...
const adx  = (h,l,c,p=14)=>{
  ...
  const smooth=a=>{ const res=[sma(a.slice(0,p),p)];
    for(let i=p;i<a.length;i++) res.push(res.at(-1)-(res.at(-1)/p)+a[i]);
    return res;
  };
...
async function buildDashboardData () {
  ...
  try{
    ...
    const mu=sma(rates,rates.length),
          sd=Math.sqrt(rates.reduce((s,x)=>s+(x-mu)**2,0)/rates.length);
    ...
    const oiPct = +(hist.filter(v=>v<=oiCurr).length/hist.length*100).toFixed(1);
    ...
    const liqR = await safeJson("https://raw.githubusercontent.com/Cydeee/Testliquidation/main/data/totalLiquidations.json");
    const btc  = (liqR.data||[]).find(r=>r.symbol==="BTC")||{};
    out.dataB={
      fundingZ, oiDelta24h:oiΔ,
  ...
  try{
    /* session ratios (last 240 × 1-hour bars) */
    ...
    h240.forEach(b=>{ const hr=new Date(+b[0]).getUTCHours(), v=+b[7];
      if(hr<8) buckets.asia.push(v); else if(hr<14) buckets.eu.push(v); else if(hr<22) buckets.us.push(v); });
    const rel=a=>a.length<21?1:+(a.at(-1)/(sma(a.slice(-21,-1),20)||1)).toFixed(2);
    out.dataD.sessionRelVol={ asia:rel(buckets.asia), eu:rel(buckets.eu), us:rel(buckets.us) };

    /* multi-window volume + CVD */
    const win={ "15m":0.25,"1h":1,"4h":4,"24h":24 }; out.dataD.cvd={};
    for(const [lbl,h]=Object.entries(win)){
      const end=Date.now(), start=end-h*3600_000;
  ...
  try{
    ...
    const lev =out.dataB.oi30dPct? Math.min(3,(out.dataB.oi30dPct-50)/10):0;
    const vF  =out.dataD.relative["15m"], vol=vF==="very high"?2:vF==="high"?1:0;
    const liq =out.dataB.liquidations||{}, diff=Math.abs((liq.long24h||0)-(liq.short24h||0)), liqScore=Math.min(2,diff/1e6);
  ...
  try{
    ...
    const vp=b=>{ const bk={}; b.forEach(r=>{ const px=(+r[2]+ +r[3]+ +r[4])/3, k=Math.round(px/100)*100; bk[k]=(bk[k]||0)+ +r[5]; });
    ...
    const last1=await safeJson(`https://api.binance.com/api/v3/klines?symbol=${SYMBOL}&interval=1m&limit=1`);
    out.dataF.price=+(+last1[0][4]).toFixed(2);

    /* anchored VWAP */
    const closes=bars1w.map(r=>+r[4]), lows52=closes.slice(-52), idx=lows52.indexOf(Math.min(...lows52));
    ...
    let num=0,den=0; daily.forEach(r=>{ const p=(+r[2]+ +r[3]+ +r[4])/3, v=+r[5]; num+=p*v; den+=v; });
    out.dataF.avwapCycle=+(num/den).toFixed(2);

    /* intraday pivot, VWAP band, 20-bar HH / LL */
    const y=bars1d.at(-2); if(y){
      const [,,high,low,close]=y, pivot=(+high+ +low+ +close)/3, R1=2*pivot-low, S1=2*pivot-high;
      const h20=await safeJson(`https://api.binance.com/api/v3/klines?symbol=${SYMBOL}&interval=1h&limit=20`);
      const HH20=Math.max(...h20.map(k=>+k[2])), LL20=Math.min(...h20.map(k=>+k[3]));
      const zero=new Date(); zero.setUTCHours(0,0,0,0);
      const mBars=await safeJson(`https://api.binance.com/api/v3/klines?symbol=${SYMBOL}&interval=1m&startTime=${zero.getTime()}&limit=1440`);
      let sv=0,sp=0,sp2=0; mBars.forEach(b=>{ const v=+b[5], px=(+b[1]+ +b[2]+ +b[3]+ +b[4])/4; sv+=v; sp+=px*v; sp2+=px*px*v; });
      const vwap=sp/sv, σ=Math.sqrt(Math.max(sp2/sv - vwap*vwap,0));
      out.dataF.levels={
        pivot:+pivot.toFixed(2), R1:+R1.toFixed(2), S1:+S1.toFixed(2),
        HH20:+HH20.toFixed(2), LL20:+LL20.toFixed(2),
        vwap:+vwap.toFixed(2), vwapUpper:+(vwap+σ).toFixed(2), vwapLower:+(vwap-σ).toFixed(2)
      };
    }
  ...
  /* G ― macro */
  try{
    const g=await safeJson(\"https://api.coingecko.com/api/v3/global\"), d=g.data;
    out.dataG={ totalMcapT:+(d.total_market_cap.usd/1e12).toFixed(2),
...
export default async function handler (request) {
  try{
    ...
    return new Response(JSON.stringify({error:\"Service unavailable\", details:err.message}),{ status:500 });
  }
}
