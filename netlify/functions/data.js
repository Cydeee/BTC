/*  Netlify Node 18 Edge Function – data.js
    ...
    Features
    ──────────────────────────────────────────────────────────────────
    • Block A  – EMA-50/200, RSI-14, ATR-pct, MACD-hist, ADX-14 (4 h)
    • Block B  – Funding-Z, OI-Δ 24 h, OI percentile, liquidations
    • Block C  – ROC-10/20 (all TFs)
    • Block D  – CVD & bull/bear volumes, relative session volume
    • Block E  – Synthetic stress index
    • Block F  – VPVR (4 h/1 d/1 w), live price, cycle-anchored VWAP,
                 intraday levels (pivot/VWAP band/HH20/LL20),
                 NEW  ➜  swings {H1,H2,L1,L2}, neckline, neckBreak
    • Block G  – Global market-cap stats
    • Block H  – Fear & Greed index
*/
...
async function buildDashboardData(){
  ...
  for(const tf of["15m","1h","4h","1d"]){
    try{
      ...
      out.dataA[tf]={ ema50:+ema(c,50).toFixed(2), ema200:+ema(c,200).toFixed(2),
  ...
  try{
    ...
    const rates=fr.slice(-42).map(r=>+r.fundingRate), mu=sma(rates,rates.length);
    ...
    const histArr=oiHist.map(o=>+o.sumOpenInterest), oiCurr=+oiNow.openInterest;
    const oi30dPct=+(histArr.filter(v=>v<=oiCurr).length/histArr.length*100).toFixed(1);
    const base24=histArr.length>25?histArr.at(-25):histArr[0];
    const oiDelta24h=base24?(((oiCurr-base24)/base24)*100).toFixed(1):null;
    ...
    out.dataB={ fundingZ, oiDelta24h, oiCurrent:+oiCurr.toFixed(2), oi30dPct,
      liquidations:{ long1h:btc.long1h||0, short1h:btc.short1h||0,
  ...
  /* ── D  Volume & CVD (sessionRelVol) ──────── */
  try{
    const h240=await safeJson(`https://api.binance.com/api/v3/klines?symbol=${SYMBOL}&interval=1h&limit=240`);
    const bucket={asia:[],eu:[],us:[]};
    h240.forEach(b=>{const hr=new Date(+b[0]).getUTCHours(),vol=+b[7];
      if(hr<8)bucket.asia.push(vol);else if(hr<14)bucket.eu.push(vol);else if(hr<22)bucket.us.push(vol);});
  ...
  try{
    ...
    const q=Math.min(2,imb/1e6); const stress=b+l+v+q;
  ...
  try{
    ...
    out.dataF={vpvr:{"4h":vp(h4),"1d":vp(d1),"1w":vp(w1)},price:+live.toFixed(2)};

    /* --- NEW swing / neckline section --- */
    try{
      const m120=await safeJson(`https://api.binance.com/api/v3/klines?symbol=${SYMBOL}&interval=1m&limit=120`);
    ...
    }catch(e){out.errors.push(`F-swing: ${e.message}`);}

    /* anchored VWAP cycle (unchanged) */
    ...
    /* intraday pivot / VWAP band / HH20 / LL20 (unchanged) */
    const y=d1.at(-2);
    if(y){
      const yH=+y[2],yL=+y[3],yC=+y[4],pivot=(yH+yL+yC)/3,R1=2*pivot-yL,S1=2*pivot-yH;
      const h1=await safeJson(`https://api.binance.com/api/v3/klines?symbol=${SYMBOL}&interval=1h&limit=20`);
      const HH20=Math.max(...h1.map(b=>+b[2])),LL20=Math.min(...h1.map(b=>+b[3]));
      ...
      let vSum=0,pvSum=0,pv2=0; vBars.forEach(b=>{const vol=+b[5],px=(+b[1]+ +b[2]+ +b[3]+ +b[4])/4;
        vSum+=vol; pvSum+=px*vol; pv2+=px*px*vol;});
      const vwap=pvSum/vSum, sigma=Math.sqrt(Math.max(pv2/vSum - vwap*vwap,0));
      out.dataF.levels={pivot:+pivot.toFixed(2),R1:+R1.toFixed(2),S1:+S1.toFixed(2),
